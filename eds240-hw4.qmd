...
km_intervals <- chernobyl_exposure %>%
  ...
  mutate(comparison = paste0(  # assuming 1 chest xray = 0.1 mGy
    ...
    "PEAK RADIATION DOSE PER HR: ", absorbed_dose_rate_micro_gray_h, "MGY",
    "\nOR", absorbed_dose_rate_micro_gray_h/0.1, "CHEST X-RAYS PER HR", sep = " ")) %>% 
  ungroup()


km_intervals$location <-c(-0.3,-0.007,-0.007,-0.007)
...
plant_path <- here("samples", "plant.png")

warning_path <- here("samples","warning.png")


# read as JPEG and as PNG
plant <- readJPEG(plant_path, native = FALSE)
 ...
 scale_x_continuous(
  ...
  theme(
    ...
    plot.title = element_text(
      ...
      size = 30, 
      family = "ko",
      margin = margin(r = 20, l = 0, unit = "pt")
    ),
  ...
  coord_cartesian(clip = "off", ylim = c(-1, 1.3), xlim = c(0,65))
...
exposure_annotated <- exposure + 
  ...
  size = 0.7
    ...
    geom_text(data = km_intervals, aes(
  ...
  geom_segment(
  aes(x = 1, xend = 64, y = 1, yend = 1),
  color = "white",
  linewidth = 0.2
) + # km line
  
  geom_segment(
  aes(x = locations, xend = locations, y = 0, yend = 1),  # Draw a vertical line at each km_measurement
  color = "white",
  linewidth = 0.2
) + 
  
  geom_text(data = km_intervals,  # km markers
  aes(x = distance_from_ch_npp_km - 3.75, y = 1.05, 
